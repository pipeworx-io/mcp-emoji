...
`tools/list` at `https://gateway.pipeworx.io/emoji/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/emoji_lookup \
  -H 'Content-Type: application/json' \
  -d '{"input":":fire:"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/emoji_lookup`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
